...
{
  options = {
    minecraft-servers = lib.mkOption {
  ...
  config =
    let
      activated = lib.filterAttrs (_: v: v.enable) config.minecraft-servers;
    in
    lib.mkIf (activated != { }) {
      ...
      systemd.tmpfiles.rules = lib.mapAttrsToList (
        n: _: "d /minecraft-servers/${n} 0755 minecraft minecraft - -"
      ) activated;
      systemd.services = lib.mapAttrs' (n: v: {
        ...
        value = lib.recursiveUpdate {
          ...
          serviceConfig = {
            WorkingDirectory = "/minecraft-servers/${n}";
